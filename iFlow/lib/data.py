...
def generate_nonstationary_sources(n_per_seg: int, 
    ...
    The distribution within each segment is given by the keyword `dist`
    ...
    @param var_bounds \lambda_i: optional, upper and lower bounds for the modulation parameter
    @param dtype: data type for data
    ...
    L = np.random.uniform(var_lb, var_ub, (n_seg, d)) # L is the std vars
    # [40, 2]
    ...
    if uncentered:
        if centers is not None:
            ...
            m = np.random.uniform(-5, 5, (n_seg, d)) # each segment should have its own center; centers should be non-overlapping??
    else:
        m = np.zeros((n_seg, d))
        # [40, 2]

    labels = np.zeros(n, dtype=dtype) # (40000,)
    if prior == 'lap':
        ...
        sources_hs = scipy.stats.hypsecant.rvs(0, 1, (n//3, d)).astype(dtype)
    ...
    for seg in range(n_seg):
        ...
        #>>> A = torch.randn((5, 3))
        #>>> A
        tensor([[-0.5555,  0.5592,  0.6082],
                [ 0.1676, -1.2115,  1.1372],
                [-0.5317,  0.0484,  0.0232],
                [-0.0569, -1.6033,  0.4842],
                [-0.0607,  0.9516,  0.2196]])
        #>>> A * torch.FloatTensor([1, 0 ,1])
        tensor([[-0.5555,  0.0000,  0.6082],
                [ 0.1676, -0.0000,  1.1372],
                [-0.5317,  0.0000,  0.0232],
...
def generate_data(n_per_seg, 
    ...
    # Both Matrix A and B are invertible since their condition numbers are bounded.
...
def save_data(path, *args, **kwargs):
    ...
    # Sb is of shape (40000, 2)
    # Xb is of shape (40000, 4)
    # Ub is of shape (40000, 40), each row of which is a one-hot coding
    # m is of shape (40, 2)
    # L is of shape (40, 2)
...
def create_if_not_exist_dataset(root='data/', 
    ...
    # 'data/tcl_1000_40_2_4_3_1_gauss_xtanh'
